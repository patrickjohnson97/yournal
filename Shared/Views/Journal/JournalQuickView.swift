    ...
    var body: some View {
        VStack(alignment: .leading){
            ...
            HStack(alignment: .top){
                Image("\(entry.emotionSelected!)-twitter").resizable().aspectRatio(contentMode: .fit).frame(height: 30).clipped().padding(6).overlay(Circle().stroke(lineWidth: 4).foregroundColor(getThemeColor(name:"Chosen", theme: theme))).padding(.trailing, 8)
                VStack(alignment: .leading){
